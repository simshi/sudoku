...
class Model(dict):
    ...
    def _build_band(self, rows):
        return '\n'.join([self._build_one_row(row) for row in rows])
    
    def __str__(self):
        self._width = len(self)>0 and max(len(v) for _,v in self.iteritems()) or 1
        split_line = '\n' + ('+'.join(['-'*(self._width*3+2)]*3)) + '\n'
        
        bands = [self._build_band(rows) for rows in ('123','456','789')]
        return split_line.join(bands) + '\n'

    def __repr__(self):
        return "Modle(%s)" % self.__str__()
